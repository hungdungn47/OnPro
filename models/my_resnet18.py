...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.wp_head = nn.ModuleList()
        self.tp_head = nn.ModuleList()
    ...
    def forward_classifier(self, x, task_id):
        x = self.wp_head[task_id](x)
        return x

    def forward_tp_head(self, x, task_id):
        x = self.tp_head[task_id](x)
        return x

    def append_wp_head(self):
        if len(self.wp_head) == 0:
            self.wp_head.append(nn.Linear(self.feature_dim, self.num_outputs).cuda())
        else:
            self.wp_head.append(deepcopy(self.wp_head[-1]))

    def append_tp_head(self):
        if len(self.tp_head) < len(self.wp_head):
            self.tp_head.append(deepcopy(self.wp_head[len(self.wp_head) - 1]))
...
def resnet18(nclasses: int, nf: int = 64):
    ...
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2], nclasses, nf=64)
    #     if pretrained:
    #         print("Loading pretrained network")
